...
int main() {
   ...
   showCans("Poured 800ccs into the empty canister", &C[5].pour(800), 1); // here
   C[6].pour(C[5]);
   showCans("Filled one with the milk from another", &C[5], 2); // here
   showCans("Poured 1500ccs of Olive oil into Olive oil canister", &C[4].pour(1500), 1);
   C[5].pour(C[4]);
   showCans("Filled can of milk with olive oil", &C[4], 2); // here
   showCans("Poured too much into olive oil canister", &C[4].pour(1500), 1);
   for (i = 3; i < 9; i++) C[i].setContent(nullptr);
   showCans("All bad", &C[3], 6); // here
   return 0;
}
